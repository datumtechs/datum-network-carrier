...
func (ma *MetadataAuthority) AuditMetadataAuthority(audit *types.MetadataAuthAudit) (libtypes.AuditMetadataOption, error) {
	...
	if nil != err {
		...
		return libtypes.AuditMetadataOption_Audit_Pending, fmt.Errorf("query metadataAuth failed, %s", err)
	}
	...
	if nil != err {
		log.WithError(err).Errorf("Failed to query local identity on MetadataAuthority.AuditMetadataAuthority(), metadataAuthId: {%s}",
			audit.GetMetadataAuthId())
		return libtypes.AuditMetadataOption_Audit_Pending, fmt.Errorf("query local identity failed, %s", err)
	}
	...
	if identity.GetIdentityId() != metadataAuth.GetData().GetAuth().GetOwner().GetIdentityId() {
		log.Errorf("Failed to verify local identity and identity of metadataAuth owner, they is not same identity on MetadataAuthority.AuditMetadataAuthority(), metadataAuthId: {%s}",
			audit.GetMetadataAuthId())
		return libtypes.AuditMetadataOption_Audit_Pending, fmt.Errorf("metadataAuth did not current own, %s", err)
	}
	...
	if rawdb.IsNoDBNotFoundErr(err) {
		...
		return libtypes.AuditMetadataOption_Audit_Pending, fmt.Errorf("query local only valid metadataAuthId by metadataId failed, %s", err)
	}
	...
	if audit.GetMetadataAuthId() == metadataAuthId {
		...
		return libtypes.AuditMetadataOption_Audit_Pending, fmt.Errorf("This metadataAuth have been already audited")
	}
	...
	case libtypes.MetadataUsageType_Usage_Period:
		if timeutils.UnixMsecUint64() >= metadataAuth.GetData().GetAuth().GetUsageRule().GetEndAt() {
			...
			//
			auditOption = libtypes.AuditMetadataOption_Audit_Refused
...
func (ma *MetadataAuthority) ConsumeMetadataAuthority(metadataAuthId string) error {
	...
	if metadataAuth.GetData().GetAuditOption() != libtypes.AuditMetadataOption_Audit_Passed {
	...
	// remove
	if metadataAuth.GetData().GetState() == libtypes.MetadataAuthorityState_MAState_Invalid {
...
func (ma *MetadataAuthority) HasValidMetadataAuth(userType libtypes.UserType, user, identityId, metadataId string) (bool, error) {
	...
	for _, auth := range metadataAuthList {
		if auth.GetUserType() == userType &&
			...
			auth.GetData().GetState() == libtypes.MetadataAuthorityState_MAState_Released {

			// filter one by one

			if auth.GetData().GetAuditOption() == libtypes.AuditMetadataOption_Audit_Refused {
				continue
			}
...
func (ma *MetadataAuthority) HasNotValidMetadataAuth(userType libtypes.UserType, user, ideneityId, metadataId string) (bool, error) {
...
func (ma *MetadataAuthority) VerifyMetadataAuth(userType libtypes.UserType, user, metadataId string) error {
	...
	case libtypes.MetadataUsageType_Usage_Period:
		usedQuo.UsageType = libtypes.MetadataUsageType_Usage_Period
		if timeutils.UnixMsecUint64() >= usageRule.GetEndAt() {
			usedQuo.Expire = true
			metadataAuth.GetData().State = libtypes.MetadataAuthorityState_MAState_Invalid
			invalid = true
...
func (ma *MetadataAuthority) QueryMetadataAuthIdByMetadataId(userType libtypes.UserType, user, metadataId string) (string, error) {
...
func (ma *MetadataAuthority) RemoveUserMetadataAuthIdByMetadataId(userType libtypes.UserType, user, metadataId string) error {
