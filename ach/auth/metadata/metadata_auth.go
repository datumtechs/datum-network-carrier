...
func (ma *MetadataAuthority) AuditMetadataAuthority(audit *types.MetadataAuthAudit) (carriertypespb.AuditMetadataOption, error) {
	...
	if nil != err {
		...
		return carriertypespb.AuditMetadataOption_Audit_Pending, fmt.Errorf("query metadataAuth failed, %s", err)
	}
	...
	if nil != err {
		log.WithError(err).Errorf("Failed to query local identity on MetadataAuthority.AuditMetadataAuthority(), metadataAuthId: {%s}",
			audit.GetMetadataAuthId())
		return carriertypespb.AuditMetadataOption_Audit_Pending, fmt.Errorf("query local identity failed, %s", err)
	}
	...
	if identity.GetIdentityId() != metadataAuth.GetData().GetAuth().GetOwner().GetIdentityId() {
		log.Errorf("Failed to verify local identity and identity of metadataAuth owner, they is not same identity on MetadataAuthority.AuditMetadataAuthority(), metadataAuthId: {%s}",
			audit.GetMetadataAuthId())
		return carriertypespb.AuditMetadataOption_Audit_Pending, fmt.Errorf("metadataAuth did not current own, %s", err)
	}
	...
	if rawdb.IsNoDBNotFoundErr(err) {
		...
		return carriertypespb.AuditMetadataOption_Audit_Pending, fmt.Errorf("query local only valid metadataAuthId by metadataId failed, %s", err)
	}
	...
	if audit.GetMetadataAuthId() == metadataAuthId {
		...
		return carriertypespb.AuditMetadataOption_Audit_Pending, fmt.Errorf("This metadataAuth have been already audited")
	}
	...
	case carriertypespb.MetadataUsageType_Usage_Period:
		if timeutils.UnixMsecUint64() >= metadataAuth.GetData().GetAuth().GetUsageRule().GetEndAt() {
			...
			metadataAuth.GetData().State = carriertypespb.MetadataAuthorityState_MAState_Invalid
			//
			auditOption = carriertypespb.AuditMetadataOption_Audit_Refused
...
func (ma *MetadataAuthority) ConsumeMetadataAuthority(metadataAuthId string) error {
	...
	if metadataAuth.GetData().GetAuditOption() != carriertypespb.AuditMetadataOption_Audit_Passed {
	...
	// remove
	if metadataAuth.GetData().GetState() == carriertypespb.MetadataAuthorityState_MAState_Invalid {
...
func (ma *MetadataAuthority) HasValidMetadataAuth(userType carriertypespb.UserType, user, identityId, metadataId string) (bool, error) {
	...
	for _, auth := range metadataAuthList {
		if auth.GetUserType() == userType &&
			...
			auth.GetData().GetState() == carriertypespb.MetadataAuthorityState_MAState_Released {

			// filter one by one

			if auth.GetData().GetAuditOption() == carriertypespb.AuditMetadataOption_Audit_Refused {
				continue
			}
...
func (ma *MetadataAuthority) HasNotValidMetadataAuth(userType carriertypespb.UserType, user, ideneityId, metadataId string) (bool, error) {
...
func (ma *MetadataAuthority) VerifyMetadataAuth(userType carriertypespb.UserType, user, metadataId string) error {
	...
	flag, err := ma.dataCenter.IsInternalMetadataById(metadataId)
	if nil != err {
	...
	case carriertypespb.MetadataUsageType_Usage_Period:
		usedQuo.UsageType = carriertypespb.MetadataUsageType_Usage_Period
		if timeutils.UnixMsecUint64() >= usageRule.GetEndAt() {
			usedQuo.Expire = true
			metadataAuth.GetData().State = carriertypespb.MetadataAuthorityState_MAState_Invalid
			invalid = true
...
func (ma *MetadataAuthority) QueryMetadataAuthIdByMetadataId(userType carriertypespb.UserType, user, metadataId string) (string, error) {
...
func (ma *MetadataAuthority) RemoveUserMetadataAuthIdByMetadataId(userType carriertypespb.UserType, user, metadataId string) error {
