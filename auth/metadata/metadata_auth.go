...
func (ma *MetadataAuthority) ConsumeMetadataAuthority(metadataAuthId string) error {
	...
	log.Debugf("TwopcMsgStart consume metadataAuth, metadataAuthId: {%s}", metadataAuthId)
...
func (ma *MetadataAuthority) VerifyMetadataAuth(userType apicommonpb.UserType, user, metadataId string) bool {
	...
	log.Debugf("TwopcMsgStart verify metadataAuth, userType: {%s}, user: {%s}, metadataId: {%s}", userType.String(), user, metadataId)
