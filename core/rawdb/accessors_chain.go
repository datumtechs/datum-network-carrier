...
func DeleteHeader(db DatabaseDeleter, hash common.Hash, number uint64) {
	if err := db.Delete(headerKey(number, hash)); err != nil {
		log.WithError(err).Error("Failed to delete header")
	}
	if err := db.Delete(headerNumberKey(hash)); err != nil {
		log.WithError(err).Error("Failed to delete hash to number mapping")
	}
}
...
func WriteBodyPB(db DatabaseWriter, hash common.Hash, number uint64, pb []byte) {
	if err := db.Put(blockBodyKey(number, hash), pb); err != nil {
		log.WithError(err).Error("Failed to store block body")
	}
}
...
func DeleteBody(db DatabaseDeleter, hash common.Hash, number uint64) {
	if err := db.Delete(blockBodyKey(number, hash)); err != nil {
		log.WithError(err).Error("Failed to delete block body")
	}
