...
func (m *Manager) recoveryNeedExecuteTask() {
	...
	if err := m.resourceMng.GetDB().ForEachNeedExecuteTask(func(key, value []byte) error {
		if len(key) != 0 && len(value) != 0 {
			...
			// add consumeSpec
			conSumeTypes := make([]uint8, len(res.GetConsumeSpec().GetConsumeTypes()))
			consumeOptions := make([]string, len(res.GetConsumeSpec().GetConsumeTypes()))
			for i, _ := range res.GetConsumeSpec().GetConsumeTypes() {
				conSumeTypes[i] = uint8(res.GetConsumeSpec().GetConsumeTypes()[i])
				consumeOptions[i] = res.GetConsumeSpec().GetConsumeOptions()[i]
			}
	...
	for {
		...
		case <-taskMonitorTimer.C:
			...
			if future > now {
				...
				taskMonitorTimer.Reset(time.Duration(0) * time.Millisecond)
			}
