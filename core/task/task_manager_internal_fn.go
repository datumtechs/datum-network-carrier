...
func (m *Manager) driveTaskForExecute(task *types.NeedExecuteTask) error {
	...
	case libcommonpb.TaskRole_TaskRole_DataSupplier, libcommonpb.TaskRole_TaskRole_Receiver:
...
func (m *Manager) publishFinishedTaskToDataCenter(task *types.NeedExecuteTask, localTask *types.Task, delay bool) {
	...
	*/
	...
	handleFn := func() {
		...
		var taskState libcommonpb.TaskState
		if isFailed {
			taskState = libcommonpb.TaskState_TaskState_Failed
		} else {
			taskState = libcommonpb.TaskState_TaskState_Succeed
		}
...
func (m *Manager) sendTaskTerminateMsg(task *types.Task) error {
	...
	sendTerminateMsgFn := func(wg *sync.WaitGroup, sender, receiver *libcommonpb.TaskOrganization, senderRole, receiverRole libcommonpb.TaskRole, errCh chan<- error) {
	...
	for i := 0; i < len(task.GetTaskData().GetDataSuppliers()); i++ {
		...
		go sendTerminateMsgFn(&wg, sender, receiver, libcommonpb.TaskRole_TaskRole_Sender, libcommonpb.TaskRole_TaskRole_DataSupplier, errCh)
	...
	for i := 0; i < len(task.GetTaskData().GetPowerSuppliers()); i++ {
		...
		go sendTerminateMsgFn(&wg, sender, receiver, libcommonpb.TaskRole_TaskRole_Sender, libcommonpb.TaskRole_TaskRole_PowerSupplier, errCh)
	...
	for i := 0; i < len(task.GetTaskData().GetReceivers()); i++ {
		...
		go sendTerminateMsgFn(&wg, sender, receiver, libcommonpb.TaskRole_TaskRole_Sender, libcommonpb.TaskRole_TaskRole_Receiver, errCh)
	}
...
func (m *Manager) storeBadTask(task *types.Task, events []*libtypes.TaskEvent, reason string) error {
	...
	task.GetTaskData().State = libcommonpb.TaskState_TaskState_Failed
...
func (m *Manager) convertScheduleTaskToTask(task *types.Task, eventList []*libtypes.TaskEvent, state libcommonpb.TaskState) *types.Task {
...
func (m *Manager) metadataPolicyRowColumn(task *types.NeedExecuteTask, localTask *types.Task) (string, error) {
	...
	if task.GetLocalTaskRole() == libcommonpb.TaskRole_TaskRole_DataSupplier {
		...
		for _, dataSupplier := range localTask.GetTaskData().GetDataSuppliers() {
			if partyId == dataSupplier.GetPartyId() {
				...
				if types.IsNotRowAndColumnData(metadata.GetData().GetFileType()) {
...
func (m *Manager) addmonitor(task *types.NeedExecuteTask, when int64) {
	...
	m.syncExecuteTaskMonitors.AddMonitor(types.NewExecuteTaskMonitor(taskId, partyId, when, func() {
		...
		// 3、 handle ExpireTask
		if localTask.GetTaskData().GetState() == libcommonpb.TaskState_TaskState_Running && localTask.GetTaskData().GetStartAt() != 0 {
			var duration uint64
...
func (m *Manager) makeTaskResultMsgWithEventList(task *types.NeedExecuteTask) *taskmngpb.TaskResultMsg {
	...
	if task.GetLocalTaskRole() == libcommonpb.TaskRole_TaskRole_Sender {
...
func (m *Manager) handleNeedExecuteTask(task *types.NeedExecuteTask, localTask *types.Task) {
	...
	if localTask.GetTaskData().GetState() == libcommonpb.TaskState_TaskState_Pending &&
		localTask.GetTaskData().GetStartAt() == 0 {

		localTask.GetTaskData().State = libcommonpb.TaskState_TaskState_Running
		...
		if err := m.resourceMng.GetDB().StoreLocalTask(localTask); nil != err {
			log.WithError(err).Errorf("Failed to update local task state before executing task on handleNeedExecuteTask(), taskId: {%s}, need update state: {%s}",
				task.GetTaskId(), libcommonpb.TaskState_TaskState_Running.String())
		}
	}
...
func (m *Manager) handleResourceUsage(keyword, usageIdentityId string, usage *types.TaskResuorceUsage, localTask *types.Task, nmls types.NetworkMsgLocationSymbol) (bool, error) {
	...
	powerCache := make(map[string]*libcommonpb.TaskOrganization)
