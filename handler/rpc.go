...
var ttfbTimeout = params.CarrierNetworkConfig().TtfbTimeout

// respTimeout is the maximum time for complete response transfer.
var respTimeout = params.CarrierNetworkConfig().RespTimeout
...
func (s *Service) registerRPC(baseTopic string, handle rpcHandler) {
	...
	s.cfg.P2P.SetStreamHandler(topic, func(stream network.Stream) {
		...
		// Close here because we may have only written a partial response.
		defer func() {
			_err := stream.Reset()
			_ = _err
		}()
