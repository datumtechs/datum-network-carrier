...
func TestStatusRPCHandler_Disconnects_OnForkVersionMismatch(t *testing.T) {
	...
	pcl2 := protocol.ID("/rosetta/carrier_chain/req/goodbye/1/ssz_snappy")
	topic = string(pcl2)
	r.rateLimiter.limiterMap[topic] = leakybucket.NewCollector(1, 1, false)
...
func TestHandshakeHandlers_Roundtrip(t *testing.T) {
	...
	// Setup streams
	pcl := protocol.ID("/rosetta/carrier_chain/req/status/1/ssz_snappy")
	topic := string(pcl)
	r.rateLimiter.limiterMap[topic] = leakybucket.NewCollector(1, 1, false)
	var wg sync.WaitGroup
	wg.Add(1)
	...
	pcl = "/rosetta/carrier_chain/req/ping/1/ssz_snappy"
	topic = string(pcl)
