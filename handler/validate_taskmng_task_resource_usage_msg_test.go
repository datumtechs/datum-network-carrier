...
import (
	...
	"github.com/Metisnetwork/Metis-Carrier/lib/netmsg/taskmng"
...
func TestValidateTwopc_ValidTaskResourceUsage(t *testing.T) {
	...
	r := &Service{
		...
		seenTaskResourceUsageMsgCache: c,
	}

	buf := new(bytes.Buffer)
	_, err = p.Encoding().EncodeGossip(buf, &taskmng.TaskResourceUsageMsg{
	...
	topic := p2p.GossipTypeMapping[reflect.TypeOf(&taskmng.TaskResourceUsageMsg{})]
	msg := &pubsub.Message{
	...
	valid := r.validateTaskResourceUsageMessagePubSub(ctx, "foobar", msg) == pubsub.ValidationIgnore
