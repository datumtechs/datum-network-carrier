...
func TestValidateTwopc_ValidTaskResourceUsage(t *testing.T) {
	...
	_, err = p.Encoding().EncodeGossip(buf, &carriernetmsgtaskmngpb.TaskResourceUsageMsg{
	...
	topic := p2p.GossipTypeMapping[reflect.TypeOf(&carriernetmsgtaskmngpb.TaskResourceUsageMsg{})]
	msg := &pubsub.Message{
