...
func (c *ConfirmTaskPeerInfo) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Offset (0) 'DataSupplierPeerInfos'
	dst = ssz.WriteOffset(dst, offset)
	for ii := 0; ii < len(c.DataSupplierPeerInfos); ii++ {
		offset += 4
		offset += c.DataSupplierPeerInfos[ii].SizeSSZ()
	}

	// Offset (1) 'PowerSupplierPeerInfos'
	dst = ssz.WriteOffset(dst, offset)
	for ii := 0; ii < len(c.PowerSupplierPeerInfos); ii++ {
		offset += 4
		offset += c.PowerSupplierPeerInfos[ii].SizeSSZ()
	}

	// Offset (2) 'ResultReceiverPeerInfos'
	dst = ssz.WriteOffset(dst, offset)
	for ii := 0; ii < len(c.ResultReceiverPeerInfos); ii++ {
		offset += 4
		offset += c.ResultReceiverPeerInfos[ii].SizeSSZ()
	}

	// Field (0) 'DataSupplierPeerInfos'
	if len(c.DataSupplierPeerInfos) > 16777216 {
		err = ssz.ErrListTooBig
		return
	}
	{
		offset = 4 * len(c.DataSupplierPeerInfos)
		for ii := 0; ii < len(c.DataSupplierPeerInfos); ii++ {
			dst = ssz.WriteOffset(dst, offset)
			offset += c.DataSupplierPeerInfos[ii].SizeSSZ()
		}
	...
	for ii := 0; ii < len(c.DataSupplierPeerInfos); ii++ {
		if dst, err = c.DataSupplierPeerInfos[ii].MarshalSSZTo(dst); err != nil {
			return
		}
	}

	// Field (1) 'PowerSupplierPeerInfos'
	if len(c.PowerSupplierPeerInfos) > 16777216 {
		err = ssz.ErrListTooBig
		return
	}
	{
		offset = 4 * len(c.PowerSupplierPeerInfos)
		for ii := 0; ii < len(c.PowerSupplierPeerInfos); ii++ {
			dst = ssz.WriteOffset(dst, offset)
			offset += c.PowerSupplierPeerInfos[ii].SizeSSZ()
		}
	...
	for ii := 0; ii < len(c.PowerSupplierPeerInfos); ii++ {
		if dst, err = c.PowerSupplierPeerInfos[ii].MarshalSSZTo(dst); err != nil {
			return
		}
	}

	// Field (2) 'ResultReceiverPeerInfos'
	if len(c.ResultReceiverPeerInfos) > 16777216 {
		err = ssz.ErrListTooBig
		return
	}
	{
		offset = 4 * len(c.ResultReceiverPeerInfos)
		for ii := 0; ii < len(c.ResultReceiverPeerInfos); ii++ {
			dst = ssz.WriteOffset(dst, offset)
			offset += c.ResultReceiverPeerInfos[ii].SizeSSZ()
		}
	...
	for ii := 0; ii < len(c.ResultReceiverPeerInfos); ii++ {
		if dst, err = c.ResultReceiverPeerInfos[ii].MarshalSSZTo(dst); err != nil {
			return
		}
	}
...
func (c *ConfirmTaskPeerInfo) UnmarshalSSZ(buf []byte) error {
	...
	// Field (0) 'DataSupplierPeerInfos'
	{
		...
		c.DataSupplierPeerInfos = make([]*TaskPeerInfo, num)
		err = ssz.UnmarshalDynamic(buf, num, func(indx int, buf []byte) (err error) {
			if c.DataSupplierPeerInfos[indx] == nil {
				c.DataSupplierPeerInfos[indx] = new(TaskPeerInfo)
			}
			if err = c.DataSupplierPeerInfos[indx].UnmarshalSSZ(buf); err != nil {
				return err
			}
			return nil
		})
	...
	// Field (1) 'PowerSupplierPeerInfos'
	{
		...
		c.PowerSupplierPeerInfos = make([]*TaskPeerInfo, num)
		err = ssz.UnmarshalDynamic(buf, num, func(indx int, buf []byte) (err error) {
			if c.PowerSupplierPeerInfos[indx] == nil {
				c.PowerSupplierPeerInfos[indx] = new(TaskPeerInfo)
			}
			if err = c.PowerSupplierPeerInfos[indx].UnmarshalSSZ(buf); err != nil {
				return err
			}
			return nil
		})
	...
	// Field (2) 'ResultReceiverPeerInfos'
	{
		...
		c.ResultReceiverPeerInfos = make([]*TaskPeerInfo, num)
		err = ssz.UnmarshalDynamic(buf, num, func(indx int, buf []byte) (err error) {
			if c.ResultReceiverPeerInfos[indx] == nil {
				c.ResultReceiverPeerInfos[indx] = new(TaskPeerInfo)
			}
			if err = c.ResultReceiverPeerInfos[indx].UnmarshalSSZ(buf); err != nil {
				return err
			}
			return nil
		})
...
func (c *ConfirmTaskPeerInfo) SizeSSZ() (size int) {
	...
	for ii := 0; ii < len(c.DataSupplierPeerInfos); ii++ {
		size += 4
		size += c.DataSupplierPeerInfos[ii].SizeSSZ()
	}

	// Field (1) 'PowerSupplierPeerInfos'
	for ii := 0; ii < len(c.PowerSupplierPeerInfos); ii++ {
		size += 4
		size += c.PowerSupplierPeerInfos[ii].SizeSSZ()
	}

	// Field (2) 'ResultReceiverPeerInfos'
	for ii := 0; ii < len(c.ResultReceiverPeerInfos); ii++ {
		size += 4
		size += c.ResultReceiverPeerInfos[ii].SizeSSZ()
	}
...
func (c *ConfirmTaskPeerInfo) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	...
	{
		subIndx := hh.Index()
		num := uint64(len(c.DataSupplierPeerInfos))
		if num > 16777216 {
			err = ssz.ErrIncorrectListSize
			return
		}
		for i := uint64(0); i < num; i++ {
			if err = c.DataSupplierPeerInfos[i].HashTreeRootWith(hh); err != nil {
				return
			}
		}
	...
	{
		subIndx := hh.Index()
		num := uint64(len(c.PowerSupplierPeerInfos))
		if num > 16777216 {
			err = ssz.ErrIncorrectListSize
			return
		}
		for i := uint64(0); i < num; i++ {
			if err = c.PowerSupplierPeerInfos[i].HashTreeRootWith(hh); err != nil {
				return
			}
		}
		hh.MerkleizeWithMixin(subIndx, num, 16777216)
	...
	{
		subIndx := hh.Index()
		num := uint64(len(c.ResultReceiverPeerInfos))
		if num > 16777216 {
			err = ssz.ErrIncorrectListSize
			return
		}
		for i := uint64(0); i < num; i++ {
			if err = c.ResultReceiverPeerInfos[i].HashTreeRootWith(hh); err != nil {
				return
			}
		}
		hh.MerkleizeWithMixin(subIndx, num, 16777216)
