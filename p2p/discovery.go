...
func (s *Service) listenForNewNodes() {
	...
	for {
		// Exit if service's context is canceld
		if s.ctx.Err() != nil {
			break
		}
...
func (s *Service) createListener(ipAddr net.IP, privKey *ecdsa.PrivateKey) (*discover.UDPv5, error) {
	// BindIP is used to specify the ip on which we will bind our listener on
	...
	if s.cfg.HostDNS != "" {
		...
		if len(ips) > 0 {
			// Use first IP returned from resolver
			firstIP := ips[0]
...
func (s *Service) createLocalNode(privKey *ecdsa.PrivateKey, ipAddr net.IP, udpPort, tcpPort int) (*enode.LocalNode, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "could not add for version entry to enr")
	}
	return intializeAttSubnets(localNode), nil
}

func (s *Service) startDiscoveryV5(addr net.IP, privKey *ecdsa.PrivateKey) (*discover.UDPv5, error) {
...
// 6) Peer's fork digest in their ENR matches that of our localnodes.
...
// This checks our set max peers in our config, and determines whether
// our currently connected and active peers are above our set max peer limit.
