...
var MaxGossipSize = params.CarrierNetworkConfig().GossipMaxSize // 1 Mib
...
func (e SszNetworkEncoder) EncodeWithMaxLength(w io.Writer, msg interface{}) (int, error) {
	...
	if uint64(len(b)) > params.CarrierNetworkConfig().MaxChunkSize {
		return 0, fmt.Errorf(
			"size of encoded message is %d which is larger than the provided max limit of %d",
			len(b),
			params.CarrierNetworkConfig().MaxChunkSize,
		)
	}
	// write varint first
...
func (e SszNetworkEncoder) DecodeWithMaxLength(r io.Reader, to interface{}) error {
	...
	if msgLen > params.CarrierNetworkConfig().MaxChunkSize {
		return fmt.Errorf(
