...
const (
	// carrierBlockWeight specifies the scoring weight that we apply to
	// our beacon block topic.
	carrierBlockWeight = 0.8
...
func defaultBlockTopicParams() *pubsub.TopicScoreParams {
	...
	return &pubsub.TopicScoreParams{
		TopicWeight:                     carrierBlockWeight,
...
func defaultAggregateSubnetTopicParams(activeValidators uint64) (*pubsub.TopicScoreParams, error) {
	subnetCount := params.CarrierNetworkConfig().AttestationSubnetCount
...
func maxScore() float64 {
	totalWeight := carrierBlockWeight + aggregateWeight + attestationTotalWeight +
