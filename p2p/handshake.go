...
// AddConnectionHandler adds a callback function which handles the connection with a newly added peer.
// It performs a handshake with that peer by sending a hello request and validating
...
func (s *Service) AddConnectionHandler(reqFunc, goodByeFunc func(ctx context.Context, id peer.ID) error) {
	...
	// This is run at the start of each connection attempt,
	// to ensure that there aren't multiple inflight connection requests
