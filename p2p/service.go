...
func NewService(ctx context.Context, cfg *Config) (*Service, error) {
	...
	// due to libp2p's gossipsub implementation not taking into account
	// previously added peers when creating the gossipsub object.
...
// Status of the p2p service. Will return an error if the
// service is considered unhealthy to indicate that this node
// should not serve traffic until the issue has been resolved.
...
// Waits for the state to be initialized, important for initializing the p2p service as p2p need
// to be aware of genesis information for peering.
