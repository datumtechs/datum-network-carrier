...
var attestationSubnetCount = params.CarrierNetworkConfig().AttestationSubnetCount

var attSubnetEnrKey = params.CarrierNetworkConfig().AttSubnetKey
...
func (s *Service) FindPeersWithSubnet(ctx context.Context, topic string, index, threshold uint64) (bool, error) {
	...
	for {
		...
		nodes := enode.ReadNodes(iterator, int(params.CarrierNetworkConfig().MinimumPeersInSubnetSearch))
...
// Reads the attestation subnets entry from a node's ENR and determines the
// committee indices of the attestation subnets the node is subscribed to.
