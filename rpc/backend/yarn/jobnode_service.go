...
func (svr *Server) ReportTaskEvent(ctx context.Context, req *pb.ReportTaskEventRequest) (*libcommonpb.SimpleResponse, error) {
	...
	if "" == strings.Trim(req.GetTaskEvent().GetPartyId(), "") {
		return &libcommonpb.SimpleResponse{ Status: backend.ErrRequireParams.ErrCode(), Msg: "require partyId"}, nil
	}
	...
	if err := svr.B.SendTaskEvent(req.GetTaskEvent()); nil != err {
		...
		return &libcommonpb.SimpleResponse{ Status: backend.ErrReportTaskEvent.ErrCode(), Msg: errMsg}, nil
	}
	return &libcommonpb.SimpleResponse{Status: 0, Msg: backend.OK}, nil
}


func (svr *Server) ReportTaskResourceUsage (ctx context.Context, req *pb.ReportTaskResourceUsageRequest) (*libcommonpb.SimpleResponse, error) {
	...
	if req.GetNodeType() != pb.NodeType_NodeType_JobNode && req.GetNodeType() != pb.NodeType_NodeType_DataNode {
		return &libcommonpb.SimpleResponse{ Status: backend.ErrRequireParams.ErrCode(), Msg: "unknown nodeType"}, nil
	}

	if nil == req.GetUsage() {
		return &libcommonpb.SimpleResponse{ Status: backend.ErrRequireParams.ErrCode(), Msg: "unknown resourceUsage"}, nil
	}
	...
	if err := svr.B.ReportTaskResourceUsage(req.GetNodeType(), req.GetIp(), req.GetPort(),
		...
		return &libcommonpb.SimpleResponse{ Status: backend.ErrReportTaskResourceExpense.ErrCode(), Msg: backend.ErrReportTaskResourceExpense.Error()}, nil
	}
